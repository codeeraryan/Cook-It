...
import { ArrowLeftIcon, ChatBubbleOvalLeftEllipsisIcon, ChevronLeftIcon, HeartIcon } from 'react-native-heroicons/solid';
...
const Profile = ({navigation}) => {
  const {theme,profileImage,userDetail}=useTheme();
  ...
  return (<View style={{flex:1,paddingTop:insets.top,backgroundColor:theme.background,}}>
   <View style={tailwind`flex-row w-full items-center py-1 gap-25 `}><TouchableOpacity onPress={()=>navigation.goBack()} style={tailwind`ml-4 p-2 bg-amber-500 rounded-full`}><ArrowLeftIcon onPress={()=>navigation.goBack()} size={20} color={'white'}/></TouchableOpacity><Text style={tailwind`text-xl w-full text-[${theme.text}]  font-semibold`}>Profile</Text></View>
      <View style={tailwind`justify-center items-center p-10`}>
        <View style={tailwind`h-30 w-30 rounded-full overflow-hidden`}><Image style={tailwind`h-[100%] w-[100%]`} source={profileImage ? { uri: profileImage } : require('../../assets/welcomee.png')}/></View>
        <Text style={tailwind`text-xl font-semibold text-[${theme.text}]`}>{userDetail.firstName?userDetail.firstName+" "+userDetail.lastName:"userXYZ"}</Text>
